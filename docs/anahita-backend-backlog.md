...
- **Como usuário autenticado, quero consultar meu próprio perfil (`GET /auth/me`).**
  - [ ] `router.py`: `GET /auth/me` autenticado via `get_current_user`, retorna `UserPublic` (sem `hashed_password`)
  - [ ] Teste: retorna o perfil do usuário do token; 401 sem token/token inválido
