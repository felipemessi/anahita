...
- **Como jogador, quero reordenar a exibição das sessões na minha ficha para organização pessoal (sem afetar a ordem oficial de `session_number`).** ✅ (2026-08-29)
  - [x] Campo de ordenação pessoal por personagem (não uma coluna compartilhada em `Session`) — ex. tabela de junção `character_id`/`session_id`/`sort_order`
  - [x] `PATCH /characters/{id}/sessions/order` (reordenação em lote)
  - [x] Testes: reordenação não afeta `session_number` global nem a ordem vista por outro personagem/jogador
  - Notas: nova tabela `character_session_orders` (migração `e4954bf4d730`, modelo `CharacterSessionOrder`) — `id`/`character_id`/`session_id`/`sort_order`, `UniqueConstraint(character_id, session_id)`, FKs `ON DELETE CASCADE`. Puramente uma preferência pessoal de exibição; não mexe em `Session.session_number` nem em qualquer coluna compartilhada.
  - Notas: `PATCH /characters/{id}/sessions/order` (`CharacterSessionOrderRequest.session_ids`, lista ordenada) é **owner-only** — `_require_own_membership`, o mesmo guard de `update_character`, diferente do GET (que também deixa o DM ver). É uma preferência pessoal, não dado de campanha; nem o DM pode setá-la para o jogador. Semântica de "substituição total": cada chamada apaga as entradas antigas do personagem e grava as novas com `sort_order = índice na lista` — um `session_id` omitido simplesmente perde a preferência salva (não é um "no-op parcial"). Todo `session_id` enviado precisa estar no conjunto retornado por `get_sessions_for_character` (422 se não estiver), e repetir um `session_id` na mesma chamada é 422.
  - Notas: `CharacterService.get_character_sessions` (o GET já existente) passou a aplicar essa ordem via `_apply_personal_session_order` — sessões com preferência salva vêm primeiro (por `sort_order`), o restante é anexado depois na ordem de `session_number` (fallback já existente, agora só usado pra "o resto"). Sem preferência salva nenhuma, comportamento idêntico ao pré-existente (só `session_number`).
  - Notas: bug descoberto e corrigido inline — apagar as linhas antigas e inserir as novas no mesmo flush do SQLAlchemy pode intercalar o INSERT antes do DELETE efetivo e violar a `UniqueConstraint`; corrigido com um `await db.flush()` explícito entre o delete e o insert em `reorder_sessions`.
...
- **Como jogador, quero que recursos de classe que geram ações (ex. Turn Undead via Canalizar Divindade) disparem a ação correspondente, não só decrementem um contador.** ✅ (2026-08-30)
  - [x] Conectar `CharacterService.use_resource` (hoje só `entry.used += 1`) ao fluxo de `declare_action` do combate — um novo `action_type` (ou reaproveitar `cast_spell_effect`/`attack_spell`) que consome o recurso e resolve o efeito mecânico correspondente numa chamada só
  - [x] Mapear, pelo menos pro caso citado (Canalizar Divindade → Turn Undead), qual efeito mecânico cada `FeatureOption` de recurso aciona (resistência de mortos-vivos, dano, etc.)
  - [x] Testes: uso do recurso em combate consome o contador **e** aplica o efeito ao(s) alvo(s) corretamente; uso fora de combate falha com erro claro (ou é tratado como bookkeeping-only, a decidir)
  - Notas: novo `ActionType.use_class_resource` em `app/combat/domain.py`, resolvido por `CombatService._resolve_class_resource` (`app/combat/service.py`) — delega o bookkeeping (decremento + registro da opção escolhida) a `CharacterService.use_resource` (mesma regra owner-only dele: um DM declarando essa ação em nome de um jogador ainda recebe 403, diferente de ataques/contests, que o DM sempre pode declarar por qualquer um — documentado no docstring de `WSDeclareActionPayload`). Mapeamento de efeito por enquanto é só `channel-divinity-turn-undead` (`Feature.index`) → `_CLASS_RESOURCE_EFFECTS`; qualquer outro recurso/opção (Rage, Sacred Weapon, Preserve Life, etc.) continua bookkeeping-only, como antes desta história. **Simplificação deliberada de área**: o app ainda não tem geometria de mapa/tokens (isso é Fase 15), então "todo morto-vivo num raio de 30 pés" virou "os participantes que o cliente listar explicitamente" via `target_id` + `additional_target_ids` — sem busca automática por raio. Cada alvo listado rola um teste de resistência de Sabedoria (ou usa `manual_save_rolls[participant_id]`, mesma convenção de rolagem manual já usada em ataques/contests) contra a DC de magia calculada a partir da classe dona da opção escolhida (`8 + proficiência + modificador da habilidade de conjuração dessa classe`, via `Feature.class_definition_id` — mesma fórmula de `CharacterService._spell_save_dc`/`engine.combat.calculate_save_dc`); falha aplica a condição `frightened` já existente do sistema de condições do combate, com `duration_rounds=10` (1 minuto a 6s/rodada). Isso é uma simplificação de rótulo, não só de área: o PHB usa o estado "turned" (foge, só pode Dash/Dodge, sem reações), que não é uma das 15 condições que este app rastreia — `frightened` foi escolhido por ser o mais próximo do sistema existente, mas não reproduz a mecânica de "turned" com fidelidade total (ex. não impede reações, não força fuga automaticamente). "Destroy Undead" (destruição automática de mortos-vivos de baixo CR a partir do nível 5 de Clériga) também não foi modelado — fora do escopo desta história, que definiu o efeito só como o teste de resistência + `frightened`. Validação de alvo (stat block ausente sem `manual_save_rolls`) roda **antes** de `CharacterService.use_resource` para não gastar a carga de um personagem quando a ação falha por um alvo não resolvível — `use_resource` comita internamente, então a ordem importa. Testes novos em `backend/tests/combat/test_class_resource_action.py` (10 casos): falha aplica `frightened` e gasta a carga; sucesso não aplica condição mas ainda gasta a carga; múltiplos alvos explícitos com resultados independentes; alvo manual exige `manual_save_rolls` (422 claro sem ele); uso sem efeito mapeado é bookkeeping-only; `resource_key` ausente é 422; atacante sem personagem (monstro/manual) é 422; opção obrigatória quando há mais de uma (herdado de `use_resource`); DM não pode gastar recurso de jogador (403, herdado); limite de usos esgotado propaga 422 pelo fluxo de combate.

- **Como jogador, quero que magias com alvo apliquem o efeito automaticamente ao alvo em combate (cura, dano com resistência), mantendo a ficha como bookkeeping-only fora de combate.** ✅ (2026-08-30)
  - [x] Auditar `declare_action`→`_resolve_attack` (`app/combat/service.py`): confirmar se magias de cura/buff (não-ataque) já aplicam efeito ao alvo, ou só as de ataque/dano com resistência
  - [x] Se cura/buff não estiverem cobertas, estender o fluxo de resolução pra aplicar o efeito (ex. HP gain) ao `target_participant_id` quando a magia não é `attack_roll`/`saving_throw`
  - [x] Confirmar e documentar que o comportamento da ficha fora de combate (`POST /characters/{id}/spells/{id}/cast`, bookkeeping-only — consome slot, define concentração, calcula DC, sem aplicar efeito a ninguém) já está correto por design, sem mudança de código necessária ali
  - [x] Testes: magia de cura em combate aplica HP ao alvo corretamente; magia de dano com resistência já testada anteriormente continua passando; cast pela ficha (fora de combate) não altera HP de ninguém
  - Notas: auditoria confirmou a lacuna — antes desta história, `declare_action`'s `attack_spell` sempre passava por `_resolve_attack`/`_resolve_and_apply_attack` (rolagem de ataque vs CA), mesmo para magias `cast_only` (SRD `action_type`) como Cure Wounds; uma magia de cura declarada assim quebrava com 422 ("No damage expression could be resolved") porque o catálogo não tem `SpellDamage` pra cura, e mesmo com uma expressão manual o fluxo aplicaria dano vs CA em vez de curar. Corrigido adicionando `CombatService._resolve_spell_effect` (`app/combat/service.py`): `_resolve_attack` agora resolve o `SpellAttackProfile` de um caster com personagem *antes* de decidir o fluxo — quando `action_type == cast_only`, delega pra `_resolve_spell_effect` em vez do to-hit; toda outra combinação (arma, monstro/manual, ou magia `attack_roll`/`saving_throw`) segue o fluxo antigo sem mudança. `_resolve_spell_effect` não faz rolagem de acerto: usa `spell_profile.target_type` (`ally` → cura, aplica diretamente a `hit_point_current` do alvo capado em `hit_point_max`; qualquer outro → dano direto, mesma aplicação de uma magia sem resistência, incluindo o `_concentration_dc` do alvo) — o valor vem do `SpellDamage` do catálogo quando a magia tem um (ex. Magic Missile, que já é auto-hit por definição SRD) ou de `manual_damage_roll` quando não tem (toda magia de cura hoje, já que Fase 8 só seedou `SpellDamage` ofensivo — a cura ainda não tem dado de dado no catálogo, então quem declara a ação rola `1d8 + MOD` etc. no cliente e informa o total). `DeclareActionResultRead` ganhou `healing_applied` (novo campo, separado de `damage_rolled`) pra o cliente não confundir cura com dano na mesma resposta. `SpellAttackProfile` (`app/queries/spell_attack.py`) ganhou `target_type`, só usado por este fluxo. Fora de combate, `CharacterService.cast_spell` já não aplicava efeito a ninguém (sem `EncounterParticipant` pra tocar) — confirmado sem mudança de código, só um teste novo provando isso (`test_cast_heal_spell_out_of_combat_does_not_change_hp`). Testes novos: `test_declare_cast_only_heal_spell_applies_hp_to_target`, `test_declare_cast_only_heal_spell_caps_at_hit_point_max`, `test_declare_cast_only_damage_spell_hits_without_attack_roll` (`backend/tests/combat/test_declare_action.py`) e `test_cast_heal_spell_out_of_combat_does_not_change_hp` (`backend/tests/characters/test_service.py`); o teste de dano com magia de ataque já existente (`test_declare_spell_attack_resolves_catalog_damage`, Fire Bolt) continua passando sem alteração. Cura/dano de magias `cast_only` continuam limitadas a um único `target_id` por declaração — não modelam Magic Missile mirando vários alvos nem Mass Cure Wounds em vários aliados de uma vez (fora do escopo desta história; a área/múltiplos alvos é a mesma simplificação documentada na história de Turn Undead acima).

- **Como jogador, quero que a duração de uma magia respeite as regras de tempo do combate (rodadas) quando em combate, e o tempo real quando fora de combate, com contador visível nos segundos finais.** ✅ (2026-08-30)
  - [x] Modelar duração de magia: quando lançada dentro de um `encounter_id`, duração em rodadas (reaproveitando o padrão de `EncounterParticipantCondition.duration_rounds`, respeitando os segundos-por-turno do encontro); quando fora de combate, duração em tempo real (`expires_at` calculado a partir de `datetime.now(UTC)` + duração da magia)
  - [x] Expandir o que hoje é só `concentrating_spell_id` (ponteiro booleano-ish) para carregar a duração/expiração ativa
  - [x] Endpoint ou campo de leitura que informe o tempo restante (rodadas ou segundos) pra UI renderizar o contador
  - [x] Testes: duração em rodadas decrementa por avanço de turno; duração em tempo real expira corretamente após o tempo passar; leitura do tempo restante bate com o esperado nos dois modos
  - Notas: novo `engine/spell_duration.py` (puro, sem framework) parseia o texto livre do SRD (`catalog_spells.duration`, ex. `"Up to 1 minute"`, `"Instantaneous"`, `"Until dispelled"`) para segundos reais — `"Up to X"` é só a marcação de concentração do SRD, não muda o parsing; `Instantaneous`/`Special`/`Until dispelled` (e qualquer texto não reconhecido) viram `seconds=None`, ou seja, duração indefinida sem relógio a rastrear. `seconds_to_rounds` converte pra rodadas de combate arredondando pra cima, a 6s/rodada — a mesma suposição de segundos-por-rodada já usada informalmente em `EncounterCondition.duration_rounds` (não havia constante nomeada antes; agora é `engine.spell_duration.ROUND_SECONDS`). Escopo deliberadamente limitado a **concentração**: o `Character` só tem um ponteiro de "efeito ativo" (`concentrating_spell_id`), então a duração/expiração foi modelada como extensão dele (`concentration_encounter_id`, `concentration_round_started`, `concentration_duration_rounds`, `concentration_expires_at`) em vez de introduzir uma tabela genérica de efeitos ativos (magias com duração mas sem concentração, ex. Longstrider, continuam sem tracking — fora do escopo desta história, que seguiu literalmente "expandir o que hoje é só `concentrating_spell_id`"). `CharacterSpellCastRequest`/`CharacterConcentrationRequest` ganham `encounter_id` opcional (não validado contra a campanha, mesma postura de `target_participant_id` — só decide o modo de tracking); `CharacterService._start_concentration` busca o `Encounter` (se houver) e escolhe rodadas vs. tempo real, `_clear_concentration` limpa os quatro campos juntos em qualquer ponto de saída (cast de outra magia de concentração, `set_concentration(spell_id=None)`). Remanescente é sempre **computado na leitura**, nunca decrementado/persistido — `app/queries/spell_duration.get_concentration_remaining` (cross-domain: characters + `Encounter.current_round`, mesmo padrão de `app.queries.character_sessions`) é chamado em todo `_to_read` (get/list/reload), devolvendo `ConcentrationRemainingRead` com `mode` (`rounds`/`seconds`/`indefinite`/`None`), `remaining_rounds`/`remaining_seconds` e `expired` — não há endpoint dedicado, o campo já vem embutido em `CharacterRead.concentration_remaining` (satisfaz o "campo de leitura" da história sem duplicar a rota). Um encontro apagado/não encontrado (`concentration_encounter_id` órfão) degrada pra `"indefinite"` em vez de erro. Comparação de `concentration_expires_at` com `datetime.now(UTC)` normaliza pra naive antes de subtrair — SQLite arredonda `DateTime(timezone=True)` pra naive UTC no round-trip, mesma técnica já usada em `CampaignService.accept_invite` pra convites expirados.
