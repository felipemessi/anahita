...
- **Lacunas remanescentes da leva acima — ainda pendentes.**
  - [ ] `PATCH /campaigns/{campaign_id}` (editar `name`/`description`/`setting`) — só o DM; hoje uma campanha só pode ser configurada na criação, sem forma de corrigir nome/descrição depois. Bloqueia a edição de "configurações gerais" da tela de settings do frontend (`app/campaigns/[campaignId]/settings/page.tsx`), que hoje só mostra os dados, read-only.
  - [ ] Endpoint em lote para resolver perfil público (`username`/`email`) de uma lista de `user_id` (ex. `GET /users?ids=`) — hoje `GET /campaigns/{id}/members` retorna só `user_id`, sem nome. Bloqueia a UI de mostrar nomes de membros em vez do UUID cru (ver `settings/page.tsx`, lista de membros).
  - [ ] `POST /catalog/{magic-items,backgrounds,feats,rules}` — criação de homebrew para as 4 categorias que ficaram fora da leva anterior (v1 do form do frontend cobria só races/classes/spells/items/monsters). `rules` também precisa ganhar `campaign_id` como query param de escopo em `GET /catalog/rules` (as outras 8 categorias já ganharam).
  - [ ] Modelar spells/equipment/features na ficha de personagem: `CharacterSpell`/`CharacterEquipment` já existem como tabelas (`app/characters/models.py`) mas não têm schema/service/router — faltam endpoints para gerenciar magias conhecidas/preparadas, itens carregados (equipped/quantity/attunement) e features resolvidas por classe/raça/talento. Bloqueia as seções "Spells"/"Equipment"/"Features" da ficha de personagem no frontend (`character-sheet.tsx`), hoje todas "em breve".
